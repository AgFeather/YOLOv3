...
for txt_file in ground_truth_files_list:
    ...
    with open(tmp_files_path + "/" + file_id + "_ground_truth.json",
              'w') as outfile:
        ...
        for idx, prediction in enumerate(predictions_data):
            ...
            if show_animation:
                    ...
                    if status == "INSUFFICIENT OVERLAP":
                        ...
                        text = "IoU: {0:.2f}% ".format(
                            ovmax * 100) + ">= {0:.2f}% ".format(
                                min_overlap * 100)
                        color = green
                    img, _ = draw_text_in_image(img, text,
                ...
                if ovmax > 0:  # if there is intersections between the bounding-boxes
                    ...
                    cv2.rectangle(img, (bbgt[0], bbgt[1]), (bbgt[2], bbgt[3]),
                                  light_blue, 2)
                    cv2.rectangle(img_cumulative, (bbgt[0], bbgt[1]),
                                  (bbgt[2], bbgt[3]), light_blue, 2)
                    cv2.putText(img_cumulative, class_name,
                                (bbgt[0], bbgt[1] - 5), font, 0.6, light_blue,
                                1, cv2.LINE_AA)
                bb = [int(i) for i in bb]
                cv2.rectangle(img, (bb[0], bb[1]), (bb[2], bb[3]), color, 2)
                cv2.rectangle(img_cumulative, (bb[0], bb[1]), (bb[2], bb[3]),
                              color, 2)
...
if draw_plot:
    ...
    draw_plot_func(pred_counter_per_class, len(pred_counter_per_class),
                   window_title, plot_title, x_label, output_path, to_show,
